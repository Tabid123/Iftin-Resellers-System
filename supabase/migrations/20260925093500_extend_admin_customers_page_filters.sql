...
declare
  ...
  v_today timestamptz := date_trunc('day', now() at time zone 'Africa/Mogadishu') at time zone 'Africa/Mogadishu';
  v_result jsonb;
begin
  if v_tenant is null then
    return jsonb_build_object('rows','[]'::jsonb,'total',0,'new_today',0,'bought_today',0,'inactive',0,'active',0);
  end if;
  ...
  order_stats as (
    select right(regexp_replace(customer_phone, '\D', '', 'g'),9) as phone_number,
           ...
           bool_or(created_at >= v_today) as bought_today
    from public.orders
  ...
  summary as (
    select count(*)::int as total,
           count(*) filter (where created_at >= v_today)::int as new_today,
  ...
  filtered as (
    ...
    where coalesce(p_filter,'all')='all'
       or (p_filter='today' and created_at >= v_today)
       or (p_filter='active' and order_count>0)
       or (p_filter='inactive' and order_count=0)
       or (p_filter='purchasedToday' and bought_today)
  ),
  filtered_count as (select count(*)::int as n from filtered),
  page_rows as (
  ...
  select jsonb_build_object(
    ...
    'total', s.total, 'filtered_total', fc.n,
    'new_today', s.new_today, 'bought_today', s.bought_today,
    'inactive', s.inactive, 'active', s.active
  )
  into v_result from summary s cross join filtered_count fc;
